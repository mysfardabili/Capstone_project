    ...
    <div className="flex flex-col gap-6" style={{ paddingBottom: '3rem', minHeight: '100%' }}>
      ...
      <div className="flex justify-between items-center" style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <h1 className="text-2xl font-bold text-text-main" style={{ margin: 0 }}>Detail Aset</h1>
        </div>
        <div className="btn-action-group">
          <button className="bg-transparent text-text-main border border-gray-200 dark:border-gray-600 px-5 py-[0.6rem] rounded-custom-md font-semibold text-sm inline-flex items-center gap-2 no-underline hover:bg-gray-100 dark:hover:bg-gray-700 transition-all" onClick={() => setShowQrModal(true)}><QrCode size={18} /> Cetak QR</button>
      ...
      {/* Tab Content */}
      <div className="bg-surface rounded-xl shadow-custom-sm border border-border overflow-hidden flex flex-col" style={{ padding: '2rem', overflow: 'visible' }}>
        {activeTab === 'info' && (
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr>
                  ...
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Tanggal</th>
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Dari Ruangan</th>
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Ke Ruangan</th>
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr>
                  ...
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Kendala / Kerusakan</th>
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Teknisi</th>
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr>
                  ...
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Tanggal Uji</th>
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Lembaga Penguji</th>
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Hasil</th>
                  <th className="bg-surface px-6 py-4 font-semibold text-sm text-text-muted border-b border-border">Jadwal Berikutnya</th>
                </tr>
              </thead>
              <tbody>
      ...
      {showQrModal && (
        <div className="fixed inset-0 bg-black/40 z-[1000] flex items-center justify-center backdrop-blur-sm">
          <div className="bg-surface rounded-[24px] p-6 max-w-[360px] w-[90%] shadow-[0_25px_50px_rgba(0,0,0,0.15)] text-center border border-gray-100 dark:border-gray-700 flex flex-col items-center">
            <h3 className="m-0 mb-4 text-[1.2rem] text-text-main font-bold">Label QR Code Aset</h3>
            ...
            <div className="border-2 border-slate-800 dark:border-slate-600 rounded-2xl p-5 w-full bg-white text-slate-900 flex flex-col items-center shadow-sm mb-6">
              ...
              <img 
                src={`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(`${window.location.origin}/technician/scan?assetId=${asset.id}`)}`} 
                alt="QR Code Preview" 
                className="w-[160px] h-[160px] my-2"
              />
              <div className="text-lg font-black tracking-tight text-slate-900 mt-2">{asset.id}</div>
              <div className="text-sm font-bold text-slate-700 mt-0.5">{asset.name}</div>
              <div className="text-xs text-slate-500 mt-0.5">Ruangan: {asset.room}</div>
            </div>
